...
(defmacro c?-with-stored ((var key store &optional default) & body)
  `(c? (bwhen-c-stored (,var ,key ,store ,default)
	 ~@body)))

(defmacro bwhen-c-stored ((var key store &optional if-not) & body)
...
(defmacro with-store-item ((item key store) & body)
  `(prog1
	(progn
	  ~@body))
...
(defmacro with-store-entry ((key store &key quiet) & body)
  `(prog1
       (progn
	 ~@body)
     (unless ,quiet
...
(defmacro with-assert-observers ((desc & asserted-observers) & body)  
  `(let ((*observers* nil))
