...
(defn md-awake (self) (eql :awake (md-state self)))

(defn fm-grandparent (md)
...
(def ^:dynamic *to-be-dbg* nil)

(defmacro make-kid (class & ia)
  (with-gensyms [c]
    `(let ((,c ,class))
       (make-instance ,c
         ~@ia
...
(defn md-census-count (&optional type)
  (when *model-pop*
  (if type
...
(defn count-model (self &key count-cells &aux (ccc 0))
