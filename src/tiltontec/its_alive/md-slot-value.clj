...
(def ^:dynamic *ide-app-hard-to-kill* t)

#+xxxxxx
(setf *c-bulk-max* most-positive-fixnum)

(defn md-slot-value (self slot-name &aux (c (md-slot-cell self slot-name)))
  (when (mdead self)
...
(defn ensure-value-is-current (c debug-id ensurer)
  ...
  (progn ;; wtrcx (:on? nil) ("evic>" (qci c) debug-id (qci ensurer))
     ...
     ((or (bwhen (nv (not (c-validp c)))
      ...
      (unless (c-currentp c) ;; happened eg when dependent changed and its observer read this cell and updated it
        (trc nil "kicking off calc-set of!!!!" (c-state c) (c-validp c) (qci c) :vstate (c-value-state c)
          :stamped (c-pulse+ c) :current-pulse @+pulse+)
    ...
    (bwhen (v (c-value c))
      #-gogo
      (when (mdead v)
        #+shhh (format t "~&on pulse+ ~a cell ~a value dead ~a" @+pulse+ c v))
      v)))
...
(defn calculate-and-set (c dbgid dbgdata)
  ...
  (flet ((body ()
           ...
           #-gogo
           (bwhen (x (cl/find c *call-stack*)) ;; circularity
             (unless nil ;; +stop+
               (let ()
                 (print `(:circler ,c))
...
(defn (setf md-slot-value) (new-value self slot-name
                              &aux (c (md-slot-cell self slot-name)))
  ...
  (cond
   ((cl/find (c-lazy c) '(:once-asked :always t))
  ...
  (without-c-dependency
      (let ((prior-state (c-value-state c))
        ...
        (when (and (not (eq propagation-code :propagate))
                (cl/find prior-state '(:valid :uncurrent))
